...
if __name__ == '__main__':
    ...
    for i in range(client_num):
        ...
        client = VFLClient(aligned_dataset=aligned_client_datasets[i], unaligned_dataset=unaligned_client_datasets[i],
                           ...
                           table_dim=8, meta_params=meta_params[i], client_id=i, dataset_type=args.dataset, label_mode=label_type)
    ...
    model_CR = CRnetCls(num_experts=3, semantic_dim=200, visual_dim=128 * 2, hidden_dim=100).to(device)
    idx_to_label = {
    ...
    model_MLP = MLP3(input_dim=128*2).to(device)
