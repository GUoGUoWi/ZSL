...
def log_memory_usage(location=""):
    ...
    logging.info(f"Memory Usage at {location}:")
    logging.info(f"RSS: {rss_mb:.2f} MB")
    logging.info(f"VMS: {vms_mb:.2f} MB")
...
if __name__ == '__main__':
    ...
    comfort_class_dict = {i: str(i) for i in range(10)}
    set_random_seed(42)
    # 基本设置
    client_num = 4
    num_classes = 10
    ...
    base_comfort_dataset = MultiViewFMNIST(data_type='train', transform=gray_test_transform)
    ...
    # 数据集划分后
    log_memory_usage("After Dataset Split")
    # # 检查基础测试数据集
