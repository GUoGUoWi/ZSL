...
from client.client_template_clip import VFLClient
from client.server_template import VFLServer
from data.cifar10_dataset import MultiViewCIFAR10
from data.comfort_dataset import ComfortDataset
from data.fmnist_dataset import MultiViewFMNIST
...
from etc.utils import setup_logging
...
def extract_clip_text_features(class_descriptions, feature_dim=512, clip_model_name="ViT-B/32"):
    """
    服务端文本特征提取函数

    Args:
        class_descriptions (dict): 类别描述字典 {class_id: description}
        feature_dim (int): 输出特征维度
        clip_model_name (str): CLIP模型名称

    Returns:
        dict: {class_id: features}，特征维度为feature_dim
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # 加载CLIP模型
    clip_model, _ = clip.load(clip_model_name, device=device)
    clip_model.eval().to(dtype=torch.float32)

    # 创建特征投影层（如果需要）
    clip_feature_dim = 512  # CLIP ViT-B/32的输出维度
    feature_projection = nn.Sequential(
        nn.Linear(clip_feature_dim, feature_dim),
        nn.ReLU()
    ).to(device) if feature_dim != clip_feature_dim else nn.Identity()

    # 将字典按照类别ID排序
    sorted_descriptions = [class_descriptions[i] for i in sorted(class_descriptions.keys())]

    # 添加详细描述
    prompts = [f"a part of {desc}" for desc in sorted_descriptions] # [f"a 3D model of a {desc}" for desc in sorted_descriptions]
    ...
    with torch.no_grad():
        ...
        # 投影到指定维度
        text_features = feature_projection(text_features)

    # 构建返回字典
    return {i: text_features[i].cpu() for i in range(len(sorted_descriptions))}
...
parser.add_argument('--allow_backbone', action='store_true')
...
if __name__ == '__main__':
    setup_logging(args=args, selected_args = ['allow_backbone', 'sup'])
    ...
    data_root = 'dataset/image/modelnet_aligned_new'
    # aligned_except_classes = comfort_aligned_except_classes
    base_comfort_dataset = MultiViewCIFAR10(data_type='train', transform=gray_test_transform)
    ...
    class_description = extract_clip_text_features(CIFAR10_DESCRIPTIONS_DETAILED)
    feature_div = [[0,2], [1,3], [0,3], [1,2]]#split_features_for_clients(base_comfort_dataset, client_num)
    ...
    for i in range(client_num):
        ...
        client = VFLClient(aligned_dataset=aligned_client_datasets[i], unaligned_dataset=unaligned_client_datasets[i],
                           ...
                           table_dim=7, meta_params=meta_params[i], client_id=i, dataset_type='fmnist', label_mode=label_type,
                           text_features=class_description, allow_backbone=args.allow_backbone)
    ...
    server = VFLServer(input_dim=dim, aligned_test_features=client_gen_datas, aligned_test_labels=client_gen_labels,
                       aligned_train_features=train_features, aligned_train_labels=aligned_labels, dataset='fmnist',
                       unaligned_features=client_unaligned_datas[0], unaligned_labels=unaligned_client_datasets[0]['Y'],
                       num_labels=num_classes, except_classes=aligned_except_classes+[0], clip=True) # except class是对其数据和全局不可见数据，第0类是全局不可见
    global_client_models, _ = server.CRNet_progress_img(client_CRnets, client_cluster_centers, client_class_prototypes, client_gen_datas,
                                                        client_gen_labels, clip=True)
    CR_epochs = 2
